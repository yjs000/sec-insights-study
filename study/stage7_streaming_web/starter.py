...
TODO 표시된 부분만 채우세요. README.md와 static/chat.html도 함께 채워야 합니다.
...
async def run_agent(question: str, send_chan: anyio.streams.memory.MemoryObjectSendStream):
    async with send_chan:
        ...
        # TODO 1: top_agent.run(user_msg=question, ctx=ctx)로 handler 받기 (await 없이)
        handler = top_agent.run(user_msg=question, ctx=ctx)  # <- 여기를 채우세요

        response_str = ""
        # TODO 2: async for event in handler.stream_events(): 로 순회하며
        #         AgentStream이면 response_str에 event.delta 누적 후 send_chan.send(response_str)
        async for event in handler.stream_events():  # <- handler.stream_events() 로 바꾸세요
            if isinstance(event, AgentStream):
                response_str += event.delta
                await send_chan.send(response_str)

        # TODO 3: await handler로 워크플로우 종료까지 대기
        await handler # <- 여기를 채우세요
...
async def index():
    # TODO 4: HTML_PATH.read_text(encoding="utf-8")로 static/chat.html 내용을 반환
    return HTML_PATH.read_text(encoding="utf-8")  # <- 여기를 채우세요


@app.get("/chat")
async def chat(q: str):
    # TODO 5: anyio.create_memory_object_stream(100)으로 send_chan, recv_chan 생성
    send_chan, recv_chan = anyio.create_memory_object_stream(100)  # <- 여기를 채우세요

    async def event_publisher():
        async with send_chan:
            task = asyncio.create_task(run_agent(q, send_chan))
            # TODO 6: async for msg in recv_chan: yield msg
            async for msg in recv_chan: yield msg # <- 여기를 채우세요
            await task
